...
export function assertCanCreateProject(ctx: ActorContext): void {
  requirePermission(
    canCreateProject(ctx),
    "Only workspace owners/admins/leaders can create projects",
    "projectCreateNotAllowed"
  );
}

export function assertCanManageProject(ctx: ActorContext): void {
  requirePermission(
    canManageProject(ctx),
    "Only project managers can manage this project",
    "projectManagerOnly"
  );
}
