...
export function LoginForm({ showDevBypass = false }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* OAuth Buttons */}
      <div className="space-y-3 rounded-xl border border-border bg-card p-6 shadow-soft">
        <button
          onClick={() => handleOAuthSignIn("github")}
          disabled={!!loading}
          className="flex w-full items-center justify-center gap-3 rounded-full border border-border bg-card px-4 py-2.5 text-sm font-medium shadow-xs transition-all duration-150 hover:bg-accent active:scale-[0.99] disabled:pointer-events-none disabled:opacity-50"
        >
          {loading === "github" ? (
