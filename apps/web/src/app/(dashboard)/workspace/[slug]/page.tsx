...
export default async function WorkspaceOverviewPage({ params }: Props) {
  ...
  const myName =
    members
      .find((m) => m.id === ctx.workspaceMemberId)
      ?.fullName?.split(" ")
      .slice(-1)[0] ?? t("home.greetingFallbackName");
      ...
      <div className="rounded-3xl border border-border bg-surface p-6 shadow-soft lg:p-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-xl font-semibold tracking-tight">
              {t("home.greeting", { name: myName })}
            </h1>
            <p className="mt-1 text-[13px] text-text-secondary">
              <span className="capitalize">{todayLabel}</span> · {workspace.name} ·{" "}
              {t("home.members", { n: members.length })}
            </p>
          </div>
          <Link
            href={`/workspace/${slug}/projects/new`}
            className={cn(buttonVariants({ variant: "dark" }), "gap-1.5 px-4")}
          >
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          <Stat
            ...
            value={myOpen}
            accent="primary"
            trend={myOpen > 0 ? t("home.trend.active") : undefined}
            icon="open"
          />
          <Stat
            ...
            trend={myInReview > 0 ? t("home.trend.waiting") : undefined}
            icon="review"
          />
          <Stat
            ...
            trend={myOverdue > 0 ? t("home.trend.needsAction") : undefined}
            icon="overdue"
          />
          <Stat
            label={t("home.stat.done")}
            value={myDone}
            accent="success"
            trend={myDone > 0 ? t("home.trend.done") : undefined}
            icon="done"
          />
        </div>
            ...
            <section>
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <h2 className="text-[15px] font-semibold">{t("home.today")}</h2>
                  <span className="rounded-full bg-secondary px-2 py-0.5 text-xs font-semibold tabular-nums text-muted-foreground">
                    {soon.length}
                  </span>
                </div>
                <span className="rounded-full border border-border/60 bg-surface-subtle px-2.5 py-1 text-[11px] font-medium text-muted-foreground">
                  {t("home.todayRange")}
                </span>
              </div>
              <div className="overflow-hidden rounded-2xl border border-border/80 bg-card shadow-soft">
                ...
                {soon.length === 0 ? (
                  <div className="px-5 py-8 text-center">
                    ...
                    <h3 className="text-sm font-semibold">{t("home.empty.title")}</h3>
                  ...
                  soon.map((task) => {
                    const due = dueMeta(task.dueDate!, todayStr, dueLabels, format);
                    return (
                      <Link
                        key={task.id}
            ...
            <section>
              ...
              {projects.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-border/80 bg-card p-8 text-center">
                  <h3 className="text-sm font-semibold">{t("home.noProjects.title")}</h3>
                  <p className="mt-1 text-xs text-muted-foreground">{t("home.noProjects.sub")}</p>
                  <Link
                    href={`/workspace/${slug}/projects/new`}
                      ...
                      >
                        ...
                        <div className="mt-3 flex items-center gap-2">
                          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-secondary">
                            <div
                              className={cn("bg-tone-progress h-full rounded-full")}
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                        ...
                        <div className="mt-2.5 flex flex-wrap items-center gap-2.5 border-t border-border/60 pt-2 text-[11px] text-muted-foreground">
                          <span className="font-medium tabular-nums">
                            {s.done}/{s.total} {t("home.doneOf")}
                          </span>
                          {s.blocked > 0 && (
                            <span className="font-semibold text-destructive">
                              {t("home.blockedN", { n: s.blocked })}
                            </span>
                          )}
                          {s.overdue > 0 && (
                            <span className="font-semibold text-warning">
                              {t("home.overdueN", { n: s.overdue })}
                            </span>
                          )}
                          {project.targetEndDate && (
                            <span className="ml-auto text-[10px]">
                              {t("home.dueAt", { d: project.targetEndDate })}
                            </span>
                          )}
                        </div>
                      </Link>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-border/80 bg-card p-4 shadow-soft">
              ...
              {activity.length === 0 ? (
                ...
                <ul className="max-h-[300px] space-y-2 overflow-y-auto pr-1">
                  {activity.map((a) => (
                    <li
                      key={a.id}
                      className="flex items-start gap-2.5 rounded-xl border border-border/40 bg-surface-subtle p-2 text-xs"
                    >
                      <span
                        className={cn(
                      ...
                      <div className="min-w-0">
                        <p className="leading-5">
                          <span className="font-semibold text-foreground">
                            {a.actorType === "human"
                              ? (a.actorName ?? t("activity.actor.someone"))
                              : t("activity.actor.agent")}
                          </span>{" "}
                          <span className="text-muted-foreground">
                            {activityEventLabel(t as unknown as EventTranslator, a.eventType)}
                          </span>
                          {a.projectName && (
              ...
              <div className="mt-3.5 grid grid-cols-3 gap-2 border-t border-border/60 pt-3 text-center">
                <MiniStat label={t("home.mini.projects")} value={projects.length} />
                <MiniStat label={t("home.mini.active")} value={activeCount} />
                <MiniStat label={t("home.mini.blocked")} value={wsBlocked} danger={wsBlocked > 0} />
              </div>
            </div>
          </div>
        </div>
...
function dueMeta(
  ...
  if (diff < 0) return { label: labels.overdue, tone: "over" };
  if (diff === 0) return { label: labels.today, tone: "soon" };
  if (diff === 1) return { label: labels.tomorrow, tone: "soon" };
  const label = format.dateTime(d, "dayMonthShort");
