...
export default async function ProjectOverviewPage({ params }: Props) {
  ...
  const doneStatusIds = new Set(
    statuses.filter((status) => status.type === "done").map((s) => s.id)
  );
    ...
    <div className="mx-auto max-w-[1240px] px-4 py-5 lg:px-6">
      ...
      <div className="rounded-3xl border border-border bg-surface p-6 shadow-soft lg:p-8">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <Stat
            label={t("project.overview.stat.totalTasks")}
            value={tasks.length}
            accent="primary"
            trend={t("project.overview.stat.totalTrend")}
            icon="tasks"
          />
          <Stat
            label={t("project.overview.stat.done")}
            value={doneCount}
            accent="success"
            trend={`${completionPct}%`}
            icon="done"
          />
          <Stat
            label={t("project.overview.stat.blocked")}
            value={blockedCount}
            accent={blockedCount > 0 ? "danger" : "muted"}
            trend={
              blockedCount > 0
                ? t("project.overview.stat.blockedNeedsAction")
                : t("project.overview.stat.blockedClear")
            }
            icon="blocked"
          />
          <Stat
            label={t("project.overview.stat.members")}
            value={projectMembers.length}
            accent="peach"
            trend={t("project.overview.stat.membersTrend")}
            icon="members"
          />
        </div>
            ...
            {phases.length > 0 && (
              <section className="rounded-2xl border border-border/80 bg-card p-4 shadow-soft">
                <h2 className="flex items-center gap-2 text-sm font-semibold text-foreground">
                  ...
                  {t("project.overview.phaseProgressTitle")}
                </h2>
                <div className="mt-3.5 space-y-1">
                  {phases.map(([name, s]) => {
                    ...
                    return (
                      ...
                      >
                        <span className="w-44 shrink-0 truncate text-xs font-semibold">
                          {name === "Chưa phân phase" ? t("project.overview.unphased") : name}
                        </span>
                        ...
                        {s.overdue > 0 && (
                          <span className="shrink-0 rounded-full bg-destructive/10 px-2 py-0.5 text-[11px] font-semibold text-destructive">
                            {t("project.overview.overdueCount", { count: s.overdue })}
                          </span>
                        )}
                      </div>
                    );
            ...
            {goals.length > 0 && (
              <section className="rounded-2xl border border-border/80 bg-card p-4 shadow-soft">
                <h2 className="flex items-center gap-2 text-sm font-semibold text-foreground">
                  ...
                  {t("project.overview.goalsMilestonesTitle")}
                </h2>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  {t("project.overview.goalsMilestonesDesc")}
                </p>
                  ...
                  {goals.map((g) => (
                    ...
                    >
                      ...
                      <div className="mt-2 flex items-center justify-between text-[11px] font-medium text-muted-foreground">
                        <span>
                          {t("project.overview.tasksProgress", { done: g.done, total: g.total })}
                        </span>
                        <span>{g.targetDate ?? t("project.overview.noTargetDate")}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {projectMembers.map((member) => (
                  <div key={member.id} className="py-2.5 first:pt-0 last:pb-0">
                    <p className="truncate text-xs font-semibold">
                      {memberNameById.get(member.workspaceMemberId) ??
                        t("project.overview.workspaceMemberFallback")}
                    </p>
