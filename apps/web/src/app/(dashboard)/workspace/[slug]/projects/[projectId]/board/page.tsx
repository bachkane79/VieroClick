...
export default async function ProjectBoardPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-[1240px] px-4 py-5 lg:px-6">
      ...
      <div className="flex min-h-0 flex-col space-y-6 rounded-3xl border border-border bg-surface p-5 shadow-soft sm:p-6">
        {/* Tinted Stat Tiles */}
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          />
          <Stat
            label={t("project.stats.todo")}
            value={todoCount}
            accent="peach"
            trend={t("project.stats.todo")}
            icon="todo"
          />
          <Stat
            label={t("project.stats.inProgress")}
            value={inProgressCount}
            accent="warning"
            trend={t("project.stats.inProgress")}
            icon="in_progress"
          />
          <Stat
